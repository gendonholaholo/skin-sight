        ...
        <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex justify-between items-center backdrop-blur-sm bg-black/20 border-b border-white/5">
            ...
            {isDashboard ? (
                <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() => {
                        // Optional: Clear session here if needed
                        window.location.href = "/";
                    }}
                    className="px-5 py-2.5 bg-red-500/10 text-red-400 border border-red-500/20 font-semibold rounded-full hover:bg-red-500/20 transition-colors flex items-center gap-2"
                >
                    <span>Log Out</span>
                </motion.button>
            ) : (
